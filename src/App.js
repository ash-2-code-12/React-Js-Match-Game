...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      secondsLeft: 60,
      currScore: 0,
      currTabId: tabsList[0].tabId,
      currDisplayId: imagesList[0].id,
    }
    this.timerId = null
  }
  ...
  endGame = () => {
    clearInterval(this.timerId)
    this.timerId = null
    this.setState({isGameOver: true, secondsLeft: 0})
  }
  ...
  selectThumbnail = imgId => {
    const {currDisplayId} = this.state
    if (imgId !== currDisplayId) {
      this.endGame()
      return
    }
    const newDisplayId = this.getRandomItem(imagesList).id

    this.setState(prevState => ({
      currDisplayId: newDisplayId,
  ...
  render() {
    // prettier-ignore
    const {secondsLeft, currScore, currTabId, currDisplayId, isGameOver} =this.state
    const currDisplayItem = imagesList.filter(
      each => each.id === currDisplayId,
    )[0]
    const currDisplayImgUrl = currDisplayItem.imageUrl
    const selectedThumbnailsList = imagesList.filter(
      each => each.category === currTabId,
    )
    console.log(currTabId)

    return (
      <div className="main-bg">
        <Navbar secondsLeft={secondsLeft} currScore={currScore} />
        {!isGameOver ? (
          <div className="game-app">
            <section className="display-section">
              <img
                className="display-img"
                src={currDisplayImgUrl}
                alt="match"
              />
            </section>
              ...
              <ul className="tabs-list">
                {tabsList.map(tabItem => (
                  <TabItem
                    ...
                    isSelectedTab={currTabId === tabItem.tabId}
                    tabItem={tabItem}
                  />
                ))}
              </ul>
            ...
            <section className="thumbnails-section">
              <ul className="thumbnails-list">
                {selectedThumbnailsList.map(imgObj => (
                  <ThumbnailItem
          ...
          <section className="score-card-section">
            <ScoreCard currScore={currScore} playAgain={this.playAgain} />
          </section>
        )}
      </div>
    )
